...
| `pak_dictionary()` | Look up official names and PBS codes |
| `pak_join()` | Join data to geometries with unmatched-row warnings |
| `pak_neighbors()` | Build spatial weights for `spdep` with Pakistan-specific boundary handling |
| `pak_crs_suggest()` | Recommend the right projected CRS for your extent |
| `pak_map()`, `pak_map_interactive()` | Quick static and interactive maps |
| `pak_centroid()`, `pak_buffer()`, `pak_distance()` | Geometric operations in km |
| `pak_points_in()` | Assign GPS points to administrative units |
