...
test_that("pk_neighbors returns correct structure", {
  ...
  expect_true("data"  %in% names(r))
  expect_s3_class(r$nb, "nb")
  expect_s3_class(r$data, "sf")
})
...
test_that("pk_neighbors disputed rejects unknown values", {
  d <- setup_polygons()
  expect_error(pk_neighbors(d$two, disputed = "exclude"),  "must be one of")
  expect_error(pk_neighbors(d$two, disputed = "flag"),     "must be one of")
})

test_that("pk_neighbors disputed = include returns full data", {
  d <- setup_polygons()
  r <- pk_neighbors(d$two, style = "queen", disputed = "include")
  expect_equal(nrow(r$data), nrow(d$two))
  expect_s3_class(r$nb, "nb")
})
